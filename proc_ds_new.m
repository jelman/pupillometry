function avgdata = proc_ds_new(filename,graphics,nobasecal,basestart,baselength);
...
%    Excel spreadsheets filename_cleaned.xlsx, filename_avgnorm.xlsx,
%    filename_avg.xlsx and filename_stats.xlsx
%
...
% Last Modified on 5/5/2014 by Sanja Kovacevic, Ph.D.
...
s = strfind(filename,'.xls');
if nobasecal % no baseline calculation
    pref = 'NoBASE_';
else
    pref = [];
end
filenameavg = sprintf('%s%s_avg.xlsx',pref,filename(1:s-1));
filenameavgnorm = sprintf('%s%s_avgnorm.xlsx',pref,filename(1:s-1));
filenameclean = sprintf('%s%s_cleaned.xlsx',pref,filename(1:s-1));
filenamest = sprintf('%s%s_stats.xlsx',pref,filename(1:s-1));

[numdata,textdata,raw] = xlsread(filename);
rmat = xlsread(filename);
Nrecordings = size(numdata,1); % number of valid recordings

header = textdata(1,:);
textdata = textdata(2:end,:); % remove header from txt data to match it with numdata

IDcol = strmatch('ID',header);
ID = raw(2:Nrecordings+1,IDcol); 
...
PupTTcol = find(strcmpi('Pupil Trial Time',header)); %first numeric column - also contains tags for data removal
Data1col = find(strcmpi('1st data pt',header)); %column with first data point

% Figure out which data this is
if strfind(header{TRcol},'PLR')
    fprintf('Processing Light Reflex data...\n');
    sf = 38.6; % sampling frequency (Hz)
    ns = 5;
    conds = 1;
    condsamp = {[1:124]};
elseif strfind(header{TRcol},'DS')
    ...
    ns = 12;
    DScol = strmatch('Bx Trial',header);
    DS = textdata(:,DScol);
    conds = [3 6 9];
...
Bad = find( strcmpi('no data',textdata(:,PupTTcol)) | strcmpi('bad data', textdata(:,PupTTcol)));
...
data.RawPupilTrials = numdata(Good,Data1col:Data1col+floor(10.2*sf)-1); % only process data recorded in 0-10.2s span
...
data.RescaleFactor=1/sf;  %sampling frequency is 38.6 Hz
...
xlswrite(filenameavgnorm,xlsdata);
...
xlswrite(filenameclean,xlsdata);
