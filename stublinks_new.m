function data = stublinks(data,graphics,lrtask,manualblinks,lowthresh,samplingrate)
% usage: data = stublinks(data,graphics,lrtask,manualblinks,lowthresh,samplingrate)
%   eg., p=stublinks(p,1,0,[1000:2000 3000:3200],30);
...
% Jia Guo added samplingrate for handling data with different sampling rate
...
if nargin<6, samplingrate=60; end
...
if lrtask
  data.BlinkTimes=data.BlinkTimes | (abs([(data.RescaleData(2:end)) (data.RescaleData(end))] - data.RescaleData) > .4*60/samplingrate)';% addapted for different sampling rate
  data.BlinkTimes=(data.BlinkTimes | (data.RescaleData'<.2*60/samplingrate));  % addapted for different sampling rate
  ...
  % mark any change greater than .5 mm in 1 sample @ 60Hz
  data.BlinkTimes=data.BlinkTimes | (abs([(data.RescaleData(2:end)) (data.RescaleData(end))] - data.RescaleData) > .5*60/samplingrate)'; % addapted for different sampling rate
  ...
  data.BlinkTimes=(data.BlinkTimes | (data.RescaleData'<(prctile(data.RescaleData,25)-2.0.*iqr(data.RescaleData)))); % was 2, now changed back to 2 as Ivan requested
  data.BlinkTimes=(data.BlinkTimes | (data.RescaleData'>(prctile(data.RescaleData,75)+2.0.*iqr(data.RescaleData)))); % was 2
  
  % kill >3mm change in 500 samples (8.3 seconds) smoothing kernal (was
  % 200) @60Hz
  kernallen=min(floor(500/60*samplingrate),length(data.RescaleData)-1); % addapted for different sampling rate
  smoothdata=conv(double(ones(1,kernallen)./kernallen),double(data.RescaleData));
  smoothdata=smoothdata((length(smoothdata)-length(data.RescaleData)+1):end);
  ...
  data.BlinkTimes=data.BlinkTimes | ((abs(smoothdata-data.RescaleData)>3)' & (smoothblinks==0));
...
%  data.BlinkTimes=data.BlinkTimes | ((abs(smoothdata-data.RescaleData)>.4)' & (smoothblinks==0));
  ...
  % JG added, choose the number of points to check based on sampling
  % frequency, the aim is to detect about 0.4mm change in 105ms, and the
  % capped number of points is set to 4 @ 60Hz as originally done, if
  % sampling rate>60Hz, then detect 0.4mm change in 66.7ms as @ 60 Hz
%  
%   for ct=1:(length(data.RescaleData)-5)
%     if ((~data.BlinkTimes(ct)) & ~data.BlinkTimes(ct+4) & (abs(data.RescaleData(ct+4)-data.RescaleData(ct))>.4*60/samplingrate)), newblinks(ct:ct+4) = 1; end
%   end
  if samplingrate<=60
      ncheck = min(floor(0.105*samplingrate),4);
  else
      ncheck = floor(4/60*samplingrate);
  end
  for ct=1:(length(data.RescaleData)-ncheck-1)
    if ((~data.BlinkTimes(ct)) & ~data.BlinkTimes(ct+ncheck) & (abs(data.RescaleData(ct+ncheck)-data.RescaleData(ct))>.4)), newblinks(ct:ct+ncheck) = 1; end
  end
...
for roundnum=1:3
  for ct=1:(length(blinkstarts)-1)
    if ((blinkstarts(ct+1)-blinkends(ct))<30*samplingrate/60) & abs(((data.RescaleData(blinkends(ct))- data.RescaleData(blinkstarts(ct)))>.4*60/samplingrate)) &  abs(((data.RescaleData(blinkends(ct+1))- data.RescaleData(blinkstarts(ct+1)))<.2*60/samplingrate)) % addapted for different sampling rate
...
for ct=1:length(blinkstarts)
  firstindex=max(1,blinkstarts(ct)-max(2,round(4*samplingrate/60))); % adapted for different sampling rate
  lastindex=min(numpts,blinkends(ct)+max(2,round(9*samplingrate/60))); % adapted for different sampling rate
